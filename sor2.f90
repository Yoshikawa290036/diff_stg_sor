subroutine sor2(nr,ntheta,acerr,peinv,dt,dthetainv,rs,thetas,dr3inv,drinv,dsinv,c,adv,coef)
    implicit none
    integer :: nr,ntheta
...
!             diff = 2.0d0*peinv*dr3inv(i)* &
!                 & ((-(rs(i)**2)*(-pc(i-1,j)+pc(i,j))*drinv(i)+(rs(i+1)**2)*(-pc(i,j)+pc(i+1,j))*drinv(i+1))+ &
!                 & dsinv(j)*dthetainv*(sin(thetas(j+1))*(-pc(i,j)+pc(i,j+1))))
!             dc(i,j) = coef(i,j)*(pc(i,j)-c(i,j)+dt*(adv(i,j)-diff))
! pc(i,j) = pc(i,j)-omega*dc(i,j)
!             err = err+dc(i,j)**2
...
!             diff = 2.0d0*peinv*dr3inv(i)* &
!                 & ((-(rs(i)**2)*(-pc(i-1,j)+pc(i,j))*drinv(i)+(rs(i+1)**2)*(-pc(i,j)+pc(i+1,j))*drinv(i+1))+ &
!                 & dsinv(j)*dthetainv*(-sin(thetas(j))*(-pc(i,j-1)+pc(i,j))))
!             dc(i,j) = coef(i,j)*(pc(i,j)-c(i,j)+dt*(adv(i,j)-diff))
! pc(i,j) = pc(i,j)-omega*dc(i,j)
!             err = err+dc(i,j)**2
!             ! err = max(err,abs(dc(i,j)))
